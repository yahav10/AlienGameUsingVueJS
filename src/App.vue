<template>
    <div id="app" class="contain">
      <GameStateStart v-if="uiState === 'start'">
        ...
        <button @click="characterSelect">Pick a character</button>

      </GameStateStart>
      <GameStateFinish v-else-if="uiState == 'Won' || uiState == 'Lost'">
        <h1 id="header">{{ uiState }}</h1>     
        <button @click="updateUI">Want to play Again?</button>
      </GameStateFinish>
      <section v-else>
...
export default {
  ...
  methods: {
    characterSelect() {
      this.$store.commit('characterSelect', this.characterinput)
